...
# fixation point
def fixation(d, ecc):
    d.rectangle((int(sz / 2) + eccentricity*ecc - f, int(sz / 2) + eccentricity*ecc + f * 3,
                 int(sz / 2) + eccentricity*ecc + f, int(sz / 2) + eccentricity*ecc - f * 3),
                fill=(0, 0, 255), outline=None)
    d.rectangle((int(sz / 2) + eccentricity*ecc - f * 3, int(sz / 2) + eccentricity*ecc + f,
                 int(sz / 2) + eccentricity*ecc + f * 3, int(sz / 2) + eccentricity*ecc - f),
                fill=(0, 0, 255), outline=None)


def stereogram(size, ecc):
    img = Image.new("RGB", (sz, sz), (lb, lb, lb))
    draw = ImageDraw.Draw(img)

    draw.rectangle((int(sz / 2) - int(ll / 2)*size, int(sz / 2) + int(f / 2),
                    int(sz / 2) + int(ll / 2)*size, int(sz / 2) - int(f / 2)),
                   fill=(0, 0, 0), outline=None)

    fixation(draw, ecc)

    basename = os.path.basename(str(size) + str(ecc) + 'ls.png')
...
def testgram(size, ecc):
    img = Image.new("RGB", (sz, sz), (lb, lb, lb))
    draw = ImageDraw.Draw(img)

    draw.rectangle((int(sz / 2) - int(f / 2), int(sz / 2) + int(ll / 2)*size,
                    int(sz / 2) + int(f / 2), int(sz / 2) - int(ll / 2)*size),
                   fill=(int(lb*1.5), 0, 0), outline=None)

    fixation(draw, ecc)

    basename = os.path.basename(str(size) + str(ecc) +'test.png')
    img.save(os.path.join(to_dir, basename), quality=100)
...
to_dir = 'materials'
os.makedirs(to_dir, exist_ok=True)
basename = os.path.basename('pedestal1.png')
img.save(os.path.join(to_dir, basename), quality=100)
...
to_dir = 'materials'
os.makedirs(to_dir, exist_ok=True)
basename = os.path.basename('pedestal2.png')
img.save(os.path.join(to_dir, basename), quality=100)
